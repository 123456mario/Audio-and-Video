...
def handle_xilica_command(cmd_raw):
    ...
    try:
        ...
        if key == "MV" or key == "MVOL":
            val = float(val_str)
            val_str_db = str(val) # Send as String dB
            
            logger.info(f"📥 Main Vol (Target Main 1): {val} dB")
            
            # Primary Target: Main 1 (as confirmed by user tests)
            send_to_wing_osc("/main/1/fdr", val_str_db)
            
            # Optional: Also send to ST if needed, but Main 1 was the specific request
            # send_to_wing_osc("/main/st/fdr", map_db_to_fader(val)) 
            return
