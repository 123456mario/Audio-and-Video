...
def send_to_xilica(cmd):
    # Try CRLF for better compatibility
    if not cmd.endswith('\r\n'): 
        cmd = cmd.rstrip() + '\r\n'
    try:
...
def process_xilica_cmd(line):
    ...
    try:
        ...
        if osc_addr and sock_osc:
            osc_data = osc_addr.encode()
            osc_data += b'\0' * ((4 - len(osc_data) % 4) % 4)
            
            if "mute" in osc_addr:
                osc_data += b",i\0\0"
                osc_data += struct.pack('>i', int(osc_val))
            else:
                osc_data += b",f\0\0"
                osc_data += struct.pack('>f', float(osc_val))
                
            sock_osc.sendto(osc_data, (WING_IP, WING_PORT))
            logger.info(f"📤 Sent to Wing: {osc_addr} {osc_val:.1f}")
...
def main():
    global sock_osc
    logger.info("🚀 STARTING BRIDGE v3.8 (Mute Diagnostic Mode)")
    ...
    def receiver():
        while True:
            try:
                ...
                # Check for mute specifically
                if b'mute' in data:
                    logger.info(f"🔍 RAW PACKET (MUTE): {data}")
                ...
                if osc_addr in POLL_TARGETS:
                    ...
                    if comma_idx != -1:
                        tags = data[comma_idx+1:].split(b'\0')[0].decode()
                        
                        # Logging tags to see what's happening
                        if 'mute' in osc_addr:
                             pass # We logged raw packet above
                        ...
                        if len(tags) >= 1:
                            ...
                            if 'f' in tags:
                                ...
                                logger.warning(f"⚠️ Unknown Tag for {osc_addr}: {tags}")
                                continue
                        ...
                        # No type tag?
                        if 'mute' in osc_addr:
                            logger.warning(f"⚠️ No type tag found for {osc_addr}")
