...
def build_osc_packet(addr_str, type_tag, val):
    # Address Padding: Must end with null, then pad to 4 bytes
    addr = addr_str.encode()
    null_pad = 4 - (len(addr) % 4)
    addr += b'\0' * null_pad
    
    # Type Tag Padding: same rule
    tag = type_tag.encode()
    null_pad = 4 - (len(tag) % 4)
    tag += b'\0' * null_pad
...
def main():
    global sock_osc
    logger.info("🚀 STARTING BRIDGE v4.1 (Padding Fix)")
    ...
    while True:
        try:
            for path in POLL_TARGETS:
                # Polling Packet also needs proper padding
                pkt = build_osc_packet(path, '', 0)
                # Strip last 4 bytes (value) and type tag for clean query if needed
                # But typically OSC query is just Address + Null padding
                
                # Correct Polling Query Construction: Address + Padding
                q = path.encode()
                pad = 4 - (len(q) % 4)
                q += b'\0' * pad
