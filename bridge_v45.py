...
def main():
    global sock_osc
    logger.info("🚀 STARTING BRIDGE v4.5 (SET 1/0 for Mute)")
                ...
                if osc_addr in POLL_TARGETS:
                    ...
                    if is_change:
                        ...
                        if "/fdr" in osc_addr:
                            ...
                            # Mute: SET 1chm 1  (Reverted to SET, but with 1/0)
                            is_on = (val >= 1)
                            x_val = 1 if is_on else 0
                            
                            if "/ch/" in osc_addr:
                                ch = osc_addr.split('/')[2]
                                xilica_cmd = f"SET {ch}chm {x_val}"
                            elif "/main/" in osc_addr:
                                xilica_cmd = f"SET mmute {x_val}"
