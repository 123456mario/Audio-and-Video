...
def map_db_to_xilica(val_norm):
    # Maps Wing Normalized (0.0 - 1.0) to Xilica (0.0 - 10.0)
    # Wing 0.0 -> Xilica 0.0
    # Wing 1.0 -> Xilica 10.0
    val = val_norm * 10.0
    if val < 0: val = 0.0
    if val > 10: val = 10.0
    return val

def map_xilica_to_db(val_xilica):
    # Maps Xilica (0.0 - 10.0) to Wing Normalized (0.0 - 1.0)
    # Heuristic: If input is <= 1.0, assume it might be already normalized 
    # (unless user really wants very low volume), but standard Xilica fader is 0-10.
    # To be safe, we assume Inputs <= 1.0 are Low Volume on a 0-10 scale, 
    # UNLESS we want to support 0-1 Logic sources.
    # Given the issue, let's treat strictly as 0-10 -> 0-1.
...
def send_to_xilica(cmd):
    # CR ONLY (\r) as per successful test_xilica_manual.py
...
def main():
    global sock_osc
    logger.info("🚀 STARTING BRIDGE v4.6 (CR only + TRUE/FALSE)")
                ...
                if osc_addr in POLL_TARGETS:
                    ...
                    if is_change:
                        ...
                        if "/fdr" in osc_addr:
                            ...
                            # Mute: SET 1chm TRUE (Reverted to TRUE/FALSE as per test_xilica_manual.py)
                            is_on = (val >= 1)
