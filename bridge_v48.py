...
def process_xilica_cmd(line):
    ...
    try:
        ...
        # Channel Mute
            ...
            try:
                ...
                if ch:
                    ...
                    v_up = val_str.upper()
                    # MUTE: Send Float 1.0/0.0 to Wing
                    is_on = (v_up == "TRUE" or v_up == "1" or v_up == "ON")
                    osc_val = 1.0 if is_on else 0.0
                    osc_type = ',f' # Changed to Float
            except: pass
            
        # Main Volume: MAIN_VOL
        elif "MAIN_VOL" in key or "MVOL" in key:
            try:
                val = float(val_str)
                osc_addr = "/main/1/fdr"
                osc_val = map_xilica_to_db(val)
        ...
        if osc_addr and sock_osc and osc_type:
            ...
            logger.info(f"📤 Sent to Wing: {osc_addr} {osc_val} (Float)")
...
def main():
    global sock_osc
    logger.info("🚀 STARTING BRIDGE v4.13 (TOGGLE Mute Fix)")
        ...
        while True:
            try:
                ...
                if osc_addr in POLL_TARGETS:
                    ...
                    val = None
                    if "mute" in osc_addr:
                         # Receive Mute from Wing
                         try:
                             val = struct.unpack('>i', data[val_offset:])[0]
                         except:
                             try: val = struct.unpack('>f', data[val_offset:])[0]
                             except: val = 0
                    else:
                    ...
                    if is_change:
                        ...
                        if "/fdr" in osc_addr:
                            # Volume: Use SET (Numeric)
                            x_val = map_db_to_xilica(val)
                            if "/ch/" in osc_addr:
                                ch = osc_addr.split('/')[2]
                                xilica_cmd = f"SET {ch}chv {x_val:.1f}" # Changed to UPPERCASE SET
                            elif "/main/" in osc_addr:
                                xilica_cmd = f"SET MAIN_VOL {x_val:.1f}"
                        
                        elif "/mute" in osc_addr:
                            # Mute: Use TOGGLE
                            # IMPORTANT: Xilica TOGGLE takes Object Name.
                            if "/ch/" in osc_addr:
                                ch = osc_addr.split('/')[2]
                                xilica_cmd = f"TOGGLE {ch}chm"
                            elif "/main/" in osc_addr:
                                xilica_cmd = f"TOGGLE mmute"
