...
def deploy():
    print("--- Starting Novastar Bridge Deployment ---")
    ...
    run_ssh_cmd(f"ssh {USER}@{HOST} 'pkill -f novastar_bridge.py || true'")

    # 4. Run the script on Pi
    print("--- Launching Novastar Bridge on Pi (Unbuffered) ---")
    # Using python3 -u to ensure logs are written to file immediately
    run_ssh_cmd(f"ssh {USER}@{HOST} 'cd ~/behringer-mixer && nohup python3 -u novastar_bridge.py > novastar_log.txt 2>&1 &'")
    
    print("\n--- Deployment Complete! ---")
    print(f"Server is running on Pi at {HOST}:10008 (UDP)")
    print("You can view logs on Pi: tail -f ~/behringer-mixer/novastar_log.txt")
