...
VOL_MAP = {
    ...
    10: 10.0   # SAFETY: Button 11 or Index 10 if sent
}
...
def process_control_cmd(line):
    ...
    if "M" in key.upper(): # MUTE
        ...
        try:
            ...
            # Wing Expects Normalized Float (0.0 ~ 1.0) for /fdr
            # Formula based on feedback logic: (db + 90) / 100
            fader_val = (db_val + 90.0) / 100.0
            fader_val = max(0.0, min(1.0, fader_val)) # Safety Clamp

            osc_addr = f"/ch/{ch}/fdr"
            osc_arg = float(fader_val) # MUST be Float, not String
            
            logger.info(f"🎚 Volume Set: {db_val}dB -> Normalized {fader_val:.4f}")
        except: pass
                ...
                if "/fdr" in path or "/mute" in path:
                     ...
                     if len(data) >= 4:
                         if "fdr" in path: 
                             ...
                             # send_to_xilica(f"SET {ch}chv {step_val:.0f}") # DISABLED TO PREVENT CRASH
                             pass
                             ...
                             # send_to_xilica(f"SETRAW {ch}chm {cmd_val}") # DISABLED TO PREVENT CRASH
                             pass
        except: pass
