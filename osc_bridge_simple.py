...
def get_channel_number(key_str):
    # Extract number from "CH1V" or "1CHV" -> 1
...
def process_xilica_cmd(line):
    ...
    try:
        ...
        key = parts[1].upper() # CH1V or 1CHV
        val_str = parts[2]
...
def main():
    global sock_osc
    logger.info("🚀 STARTING BRIDGE v3.5 (Regex Parsing Fix)")
                        ...
                        if len(tags) >= 1:
                            ...
                            if is_change:
                                ...
                                if "/fdr" in osc_addr:
                                    ...
                                    if "/ch/" in osc_addr:
                                        ch = osc_addr.split('/')[2]
                                        # Use standard ch1v format now
                                        xilica_cmd = f"set ch{ch}v {x_val:.1f}"
                                    ...
                                    if "/ch/" in osc_addr:
                                        ch = osc_addr.split('/')[2]
                                        xilica_cmd = f"set ch{ch}m {x_str}"
                                    elif "/main/" in osc_addr:
