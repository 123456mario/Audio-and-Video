...
# Accurate direction mapping based on PTS protocol
def get_pts_command(direction, cam_id=""):
    d = direction.upper()
    ...
    # Home Position -> Use PRESET 1 (#R00)
    # PRESET1 Button -> User Preset 2 (#R01)
    # PRESET2 Button -> User Preset 3 (#R02)
    # PRESET3 Button -> User Preset 4 (#R03)
    if d == "HOME":     return "#R00"
    if d == "PRESET1":  return "#R01"
    if d == "PRESET2":  return "#R02"
    if d == "PRESET3":  return "#R03"
...
def create_camera_session(cam_ip):
    try:
        ...
        log_message(f"Error creating session for {cam_ip}: {e}")
        return None
...
def control_ptz_http(cam_ip, command, is_warming=False, queue_time=None):
    ...
    if not session:
        ...
        session = create_camera_session(cam_ip)
        if session:
            cam_sessions[cam_ip] = session
        else:
            if not is_warming: log_message(f"[ERROR] No session for {cam_ip}")
            return False
...
def camera_worker(cam_id):
    cam_ip = CAM_IPS.get(cam_id)
    if not cam_ip:
        log_message(f"[ERROR] Invalid ID {cam_id}")
        return

    log_message(f"[WORKER] {cam_id} thread started ({cam_ip})")
    ...
    while True:
        try:
            ...
            # Format: 'CAM1 LEFT ON' or 'CAM2 PRESET1 ON'
            parts = cmd_text.split()
            if len(parts) < 3: continue
            
            # Validate Cam Name matches Worker
            msg_cam = parts[0]
            if msg_cam != cam_id: continue
            ...
            raw_cmd = get_pts_command(direction, cam_id)
            if not raw_cmd: 
                log_message(f"[WARN] Unknown direction: {direction}")
                continue
            
            # Determine Mode & Set Lock
            if direction in ["HOME", "PRESET1", "PRESET2", "PRESET3"]:
                active_mode[cam_id] = "PRESET"
                preset_lock_until[cam_id] = time.time() + 5.0 # Lock stops for 5s
                log_message(f"[PRESET] {cam_id} -> {direction}")
            else:
                active_mode[cam_id] = "MANUAL"
                preset_lock_until[cam_id] = 0 # Unlock immediately for manual moves
            
            # Execute Command
            # Priority: UDP for speed, HTTP for reliability
            if direction.startswith("REL_") and "ZOOM" not in direction:
                # Short movement (Logic inputs)
                control_ptz_udp(cam_ip, raw_cmd)
                control_ptz_http(cam_ip, raw_cmd, queue_time=queue_time)
            elif direction in ["ZOOMIN", "ZOOMOUT", "REL_ZOOMIN", "REL_ZOOMOUT"]:
                 # Zoom needs reliable HTTP usually
                 control_ptz_udp(cam_ip, raw_cmd)
                 control_ptz_http(cam_ip, raw_cmd, queue_time=queue_time)
            elif active_mode[cam_id] == "PRESET":
                 # Pesets work best with HTTP
                 control_ptz_http(cam_ip, raw_cmd, queue_time=queue_time)
            else:
                 # Continuous Manual
                 control_ptz_udp(cam_ip, raw_cmd)
            ...
            log_message(f"[WORKER ERROR {cam_id}]: {e}")
            time.sleep(1.0)
...
def keep_warm():
    while True:
        ...
        time.sleep(4) # Increased freq slightly
...
while True:
    try:
        if time.time() - last_heartbeat > 30:
            log_message(f"HEARTBEAT: PTZ Bridge Alive (Port {PORT})")
            last_heartbeat = time.time()
        ...
        if len(parts) >= 3:
            ...
            if cam_id in command_queues:
                if not command_queues[cam_id].full():
                    ...
                    log_message(f"[DROP] Queue full for {cam_id}")
            else:
                log_message(f"[WARN] Unknown Camera ID: {cam_id}")
