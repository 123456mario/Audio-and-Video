-- [TIMER TYPE 1: SHOW TIMER]
-- Behavior: Counts to 100% and STOPS.
-- Usage: For show progress bars. Requires manual reset via Pin 1 (Power).
...
-- STATE PERSISTENCE
-- ========================================================
if G_Show_SysState == nil then G_Show_SysState = STATE_IDLE end
if G_Show_Timer == nil then G_Show_Timer = 0 end
if G_Show_LastTickState == nil then G_Show_LastTickState = false end
...
if Is_Power_On == false then
    -- [RESET LOGIC]
    G_Show_SysState = STATE_IDLE
    G_Show_Timer = 0
else
    if G_Show_SysState == STATE_IDLE then
        G_Show_SysState = STATE_WAIT
        G_Show_Timer = 0
    elseif G_Show_SysState == STATE_WAIT then
        -- Tick Detection
        ...
        -- Increment
        if IsTickHigh and (G_Show_LastTickState == false) then
            G_Show_Timer = G_Show_Timer + 1
        end
        G_Show_LastTickState = IsTickHigh
        
        if G_Show_Timer >= WAIT_DURATION then
            G_Show_Timer = WAIT_DURATION
            G_Show_SysState = STATE_DONE
        end

    elseif G_Show_SysState == STATE_DONE then
        -- [STOP MODE: IMPORTANT]
        -- Stays here at 100%. Does NOT reset automatically.
        -- User must toggle Pin 1 (Power) to Reset.
    end
end
...
-- ========================================================
local out_blink = 0
local out_progress = G_Show_Timer / WAIT_DURATION
if out_progress > 1.0 then out_progress = 1.0 end
local val_percent = math.floor((out_progress * 100) + 0.5)

if G_Show_SysState == STATE_WAIT then
    if (G_Show_Timer % BLINK_PERIOD) == 0 then out_blink = 1 end
end
...
SetOut(2, (G_Show_SysState == STATE_IDLE and 1 or 0), "Show_Start")
SetOut(3, (G_Show_SysState == STATE_DONE and 1 or 0), "Show_Next")
...
local debug_str = string.format("State:%d | Timer:%d | Pwr:%s | Tick:%s | Blink:%d", 
    G_Show_SysState, G_Show_Timer, tostring(Is_Power_On), tostring(In_Tick), out_blink)
