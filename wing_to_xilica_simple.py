...
def handle_xilica_command(cmd_raw):
    ...
    try:
        ...
        if key == "MV" or key == "MVOL":
            val = float(val_str)
            logger.info(f"📥 Main Vol: {val} dB")
            send_to_wing_osc("/main/st/fdr", map_db_to_fader(val)) # Main uses Normalized usually?
            # Actually Main Fader on Wing is usually /main/st/fdr or /main/st/mix/fader
            # And it expects dB if we use fdr? Let's check docs later. 
            # For now assume same dB rule or standard.
            # Wing Main Fader often expects normalized 0-1 for /main/st/mix/fader?
            # Let's map dB to 0-1 just in case for Main, OR send dB if it supports it.
            # Safest: Send /main/st/mix/fader (0-1) derived from dB
            norm = map_db_to_fader(val)
            send_to_wing_osc("/main/st/mix/fader", norm)
            return

        if key == "MMUTE":
            is_muted = (val_str == "TRUE" or val_str == "1" or val_str == "ON")
            logger.info(f"📥 Main Mute: {is_muted}")
            val_int = 1 if is_muted else 0
            send_to_wing_osc("/main/st/mix/on", 0 if is_muted else 1) # Inverted for mix/on
        ...
        if match:
            ...
            if key.endswith("V") or "VOL" in key or "FDR" in key:
                 db_val = float(val_str)
                 logger.info(f"📥 Xilica Vol: Ch {ch_num} ({key}) -> {db_val} dB")
...
def handle_mute(address, *args):
    ...
    try:
        ...
        if len(parts) >= 3 and parts[1] == 'ch':
            ...
            # Xilica: Use UPPERCASE 'TRUE' / 'FALSE'
...
def handle_fader(address, *args):
    ...
    Address: /ch/1/fdr -> SET 1CHV -10.0
    """
    try:
        if not args: return
        val = args[0]
        logger.info(f"Rx FADER: {address} {val}")
        ...
        if len(parts) >= 3 and parts[1] == 'ch':
            ...
            # Key: 1CHV (UPPERCASE)
            cmd = f"SET {ch_num}CHV {db_val:.1f}"
            send_to_xilica(cmd)
...
def handle_main_fader(address, *args):
    """ Main Fader: SET MV -10.0 """
    try:
        if not args: return
        val = args[0]
        db_val = map_fader_to_db(val)
        send_to_xilica(f"SET MV {db_val:.1f}")
    except: pass
